...
// `crew_store.py`, and the `escalated` carve-out in particular is invisible until
// a fleet quietly stops picking up work because one unanswered escalation is
// holding a slot.
import {
  issueRadarApi,
  CREW_PHASES,
...
describe('crew reads — url and query construction', () => {
  ...
  it('reads escalations across every crew in the repo', async () => {
    await issueRadarApi.crewEscalations(REF)
    expect(calledUrl()).toBe('/api/apps/issue-radar/crews/escalations?owner=o&repo=r')
  })
...
describe('crew writes — method, route and body', () => {
  ...
  it('records work with PUT, carrying crew id, issue number and the flat patch', async () => {
    ...
    expect(calledInit().method).toBe('PUT')
    expect(sentBody()).toEqual({
      owner: 'o', repo: 'r', id: 'c_beef', number: 2187,
  ...
  it('keeps an explicit null in a work patch', async () => {
    ...
    await issueRadarApi.recordCrewWork(REF, 'c_beef', 7, { pr_number: null, escalation: null })
    const body = sentBody()
  ...
  it('sends guidance for one issue', async () => {
    await issueRadarApi.sendCrewGuidance(REF, 'c_beef', 2187, 'take option B')
    expect(calledUrl()).toBe('/api/apps/issue-radar/crew/guidance')
    expect(calledInit().method).toBe('POST')
    expect(sentBody()).toEqual({
      owner: 'o', repo: 'r', id: 'c_beef', number: 2187, text: 'take option B',
    })
  })
  ...
  it('merges protocol settings with PUT on the repo-wide route', async () => {
    ...
    expect(calledInit().method).toBe('PUT')
    expect(sentBody()).toEqual({ owner: 'o', repo: 'r', claim_ttl_hours: 24 })
  })
  ...
  it('declares a json content type on every write', async () => {
    const writes: Array<() => Promise<unknown>> = [
      ...
      () => issueRadarApi.sendCrewGuidance(REF, 'c_1', 1, 'go'),
...
describe('crew calls carry the session cookie', () => {
  it('sets same-origin credentials on every crew read and write', async () => {
    ...
    const calls: Array<() => Promise<unknown>> = [
      ...
      () => issueRadarApi.crewEscalations(REF),
      () => issueRadarApi.createCrew(REF, { name: 'Leo' }),
      () => issueRadarApi.updateCrew(REF, 'c_1', { enabled: false }),
      () => issueRadarApi.retireCrew(REF, 'c_1'),
      () => issueRadarApi.recordCrewWork(REF, 'c_1', 1, { phase: 'claimed' }),
...
describe('phase classification mirrors crew_store.py', () => {
  ...
  it('ages only pre-PR phases toward the claim ttl', async () => {
    // A parked PR or an open escalation is stronger evidence of a live claim than
    // any heartbeat, so those phases must NOT age toward the TTL.
    ...
    expect(TTL_ACTIVE_PHASES.has('escalated')).toBe(false)
  })
  ...
  it('draws every classified phase from the phase list', async () => {
    ...
    expect(CREW_PHASES).toHaveLength(14)
  })
  ...
  it('countsTowardOpen excludes escalated so a parked item frees its slot', async () => {
    // THE carve-out: an item waiting on a human must not consume one of the
    // crew's `max_open` slots, or a single unanswered escalation stops the crew
    // taking any other work. `escalated` is bounded by `max_escalated` instead.
    expect(countsTowardOpen('escalated')).toBe(false)
    expect(TERMINAL_PHASES.has('escalated')).toBe(false)
  })

  it('countsTowardOpen counts every other unfinished phase', async () => {
    const counted = CREW_PHASES.filter(
      (p) => !TERMINAL_PHASES.has(p) && p !== 'escalated',
    )
