/**
 * CrewPageView — the five assertions that protect the page's meaning rather than
 * its markup.
 *
 * Two of them exist because the number alone is misleading:
 *
 *   • "3 / 3" does not say the crew has STOPPED taking issues, so the at-limit
 *     note must appear exactly at the boundary and never below it.
 *   • an "Escalated 1" card next to a "3 / 3" card reads as 4 of 3 slots used
 *     unless the card states that an escalated item is uncounted — an item parked
 *     on a human decision must not consume a work slot.
 *
 * The other three protect the 24h boundary (a row must not sit on the wrong side
 * of the Earlier divider), `next` being rendered in FULL (the column is a
 * resumable intent, useless truncated), and the pause payload.
 *
...
/** The default page payload: 3 open items against max_open 3, one escalated. */
function payload(over: Record<string, unknown> = {}) {
  return {
    ...
    items: [
      ...
      item(2259, 'escalated', { last_progress_at: ago(9) }),
    ],
...
describe('CrewPageView — slot accounting', () => {
  it('shows the at-limit note only once open work items reach max_open', async () => {
    api.crew.mockResolvedValue(payload({ counts: { open: 3, escalated: 1 } }))
    renderPage()
  ...
  it('does not claim the limit is reached while a slot is still free', async () => {
    api.crew.mockResolvedValue(payload({ counts: { open: 2, escalated: 1 } }))
    renderPage()
  ...
  it('states on the escalated card that the count is not charged to the limit', async () => {
    renderPage()

    // Load-bearing next to a 3/3 card: without this line the two numbers read as
    // 4 items in 3 slots.
    const note = await screen.findByTestId('stat-escalated-note')
    expect(note).toHaveTextContent(copy('escalated_note'))
    expect(screen.getByTestId('stat-escalated')).toHaveTextContent('1')
  })
})
...
describe('CrewPageView — the next column', () => {
  ...
  it('lists exactly the slot-consuming items, so the table matches the ratio', async () => {
    ...
    // Escalated is NOT slot-consuming (`countsTowardOpen`), so it must not appear
    // in a table whose count is the "3 / 3" stat.
