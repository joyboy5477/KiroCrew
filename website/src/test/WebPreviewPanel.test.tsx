...
import WebPreviewPanel, { normalizeUrl, setSessionPreviewUrl, setSessionPreviewPending, isolatePreviewHost, withCacheBuster, PREVIEW_ENABLE_BROWSE_EVENT, BROWSE_MODE_EVENT, BROWSE_MODE_REQUEST_EVENT } from '../components/WebPreviewPanel'
...
describe('WebPreviewPanel — native browser transport', () => {
  ...
  it('pulls the current consent state on mount instead of defaulting to off', async () => {
    // Regression: BROWSE_MODE_EVENT is a push, so a panel that mounts AFTER it
    // (every agent-triggered open) kept its `false` default and mirrored that
    // into the main process, revoking a grant the user had already given.
    // The panel must ASK on mount; ordering then cannot matter.
    const seen: string[] = []
    const onRequest = () => seen.push('requested')
    window.addEventListener(BROWSE_MODE_REQUEST_EVENT, onRequest)
    try {
      installNativeBridge(true)
      renderWithProviders(<WebPreviewPanel sessionKey="sess-1" />)
      await waitFor(() => expect(seen).toContain('requested'))
    } finally {
      window.removeEventListener(BROWSE_MODE_REQUEST_EVENT, onRequest)
    }
  })
