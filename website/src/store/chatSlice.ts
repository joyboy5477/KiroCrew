    ...
    sseSideQueue(state, action: PayloadAction<{ slot: string; action: 'push' | 'edit' | 'cancel' | 'drain'; queue_id: string; content?: string; ts?: number; front?: boolean; steer_id?: string; raw?: boolean; suppressRelease?: boolean }>) {
      ...
      if (kind === 'edit') {
        ...
        if (kind === 'cancel') {
          ...
          // silently losing one.
          // Another tab's cancel: drop the card here, but leave the question to the tab that
          // actually cancelled. Without this every open dashboard pasted the same question
          // into its own composer, so the user saw it N times and could send it twice.
          if (released && !suppressRelease) {
