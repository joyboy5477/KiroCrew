  ...
  return (
    <aside style={{ width }} className="flex-shrink-0 flex flex-col min-h-0 py-2 gap-2">
      ...
      <AccordionSection
        ...
        // Return to the crews page you were last on (Your Desk, or one crew) —
        // `crewView` is persisted, so resetting it here would discard the one
        // thing the section remembers. Same contract as Dashboards above.
        onToggle={() => openCrews()}
        badge={crewCounts.needs_you > 0
          ? (
            <Badge
              variant="err"
              className="text-[11px] px-1.5 py-0 leading-[16px] tabular-nums"
              data-testid="crews-needs-you"
              // The rail is the ONLY always-visible surface: this count is a
              // crew stopped dead on a human decision, and the crews page is
              // per-repo, so without it a blocked crew is invisible until you
              // happen to open that repo's crews list.
              title={i18nT('apps.issueRadar.views.crews.rail_badge_title', { n: crewCounts.needs_you })}
            >
...
/** Body of the "Crews" accordion section: the two destinations (Your Desk, then
 * one row per crew by name), then Sort and Filters over the roster — the crew
 * analogue of FiltersSection / PrFiltersSection, and the reason those controls are
 * NOT in column 2: the rail holds how you narrow a list, the list column holds the
 * list.
 *
 * The roster rows here are navigation only — no status dot, phase word or work
 * summary, even though the roster is already loaded. Column 2 is where state is
 * read, and the rail is the surface that has to stay legible at 220px. The one
 * exception is the needs-a-decision COUNT on the section header, which is there
 * because it must survive the section being collapsed. */
function CrewsSection() {
  ...
  const FILTER_LABEL: Record<CrewFilter, string> = {
    ...
    needs_you: i18nT('apps.issueRadar.views.crews.filter_needs_you'),
  ...
  const FILTER_COUNT: Record<CrewFilter, number> = {
    ...
    needs_you: crewCounts.needs_you,
    paused: crewCounts.paused,
  }

  return (
    <div className="px-3 pt-1">
      {/* Sort and filters only — no destinations. Both of this surface's
          destinations already live in column 2: Your Desk is the card pinned above
          the roster, and a crew is its own card. Repeating either here is a second
          copy of one list, and the copy without the state. The issues and PR
          sections set the shape: the rail narrows a list, the list column holds
          it. */}
      <div className="pt-1">
