...
/** `max_open` / `max_escalated` bounds, mirroring `_validated_crew_patch`, which
 *  DROPS an out-of-range value rather than clamping it — so a 0 typed here would
 *  silently keep the old number if it were not clamped before sending. */
const SLOT_MIN = 1
const SLOT_MAX = 20
...
function draftFromCrew(crew: Crew | null | undefined): Draft {
  if (!crew) {
    return {
      ...
      maxEscalated: DEFAULTS.maxEscalated,
  ...
  return {
    ...
    maxEscalated: crew.max_escalated,
  ...
  const submit = useMutation({
    mutationFn: async () => {
      if (crew) {
        ...
        if (draft.maxEscalated !== crew.max_escalated) p.max_escalated = draft.maxEscalated
      ...
      const spec: CrewSpec = {
        ...
        max_escalated: draft.maxEscalated,
      ...
      >
        ...
        <DialogBody>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Field
              id="crew-editor-max-escalated"
              label={t('apps.issueRadar.views.crews.editor.max_escalated_label')}
              hint={t('apps.issueRadar.views.crews.editor.max_escalated_hint')}
            >
              <Input
                id="crew-editor-max-escalated"
                data-testid="crew-editor-max-escalated"
                className="w-full tabular-nums"
                type="number"
                min={SLOT_MIN}
                max={SLOT_MAX}
                value={draft.maxEscalated}
                onChange={e =>
                  patch('maxEscalated', clampSlot(e.target.value, draft.maxEscalated))
                }
              />
            </Field>
        ...
        <DialogFooter>
          ...
          <Btn type="button" onClick={onClose} className="py-1.5">
