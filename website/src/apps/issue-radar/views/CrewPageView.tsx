 ...
 *   3. stat row     slot usage, 24h throughput, replies waited on, escalations
 ...
 * `crew_store.py`'s frozensets of the same names, and they deliberately do not
 * coincide (an `escalated` item is neither terminal nor slot-consuming). A local
...
export default function CrewPageView({ crewId, onEdit }: CrewPageViewProps) {
  ...
  const counts = detail.data?.counts ?? { open: 0, escalated: 0 }
  ...
  // the row count and the "N / max" stat can never disagree. An escalated item is
  // deliberately absent from both: it is waiting on a human, and it is surfaced on
  // Your Desk instead of holding a slot here.
  ...
  const stateLabel = paused
    ...
    : newestLive?.phase ?? t('apps.issueRadar.views.crews.page.state_idle')
  ...
  return (
    <div className="px-6 pt-4 pb-6 flex flex-col gap-4" data-testid="crew-page">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-2">
            <Badge variant={stateVariant} data-testid="crew-state">{stateLabel}</Badge>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        />
        {/* An escalated item is parked on a HUMAN decision. It must not consume one
          * of `max_open`, or a single unanswered question stops the crew picking up
          * anything else — so the card says so, where the number is read. */}
        <StatBlock
          testId="stat-escalated"
          label={t('apps.issueRadar.views.crews.page.escalated')}
          value={fmtNumber(counts.escalated)}
          colorClass={counts.escalated > 0 ? 'text-danger' : undefined}
          note={t('apps.issueRadar.views.crews.page.escalated_note')}
        />
      </div>
            ...
            <tbody>
              {openItems.map((item) => (
                <tr key={item.number} data-testid={`work-item-${item.number}`}>
                  ...
                  <td className={`${TD} whitespace-nowrap`}>
                    <Badge variant={phaseVariant(item.phase)}>{item.phase}</Badge>
                  </td>
