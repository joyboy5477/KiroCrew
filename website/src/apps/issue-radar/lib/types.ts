 ...
 * split (roster in column 2, Your Desk / one crew's page in column 3). */
...
/** What the crews main area is showing: the shared "Your Desk" page (escalations
 * + repo-wide protocol settings), or one crew's own page. The column-2 crew list
 * drives this — the pinned first row sets `{kind:'desk'}`, and each crew row sets
 * `{kind:'crew'}` with that crew's id. */
export type CrewView = { kind: 'desk' } | { kind: 'crew'; id: string }
 ...
 * not survive either — see `context.tsx`, which drops the selection once the crew
 * list has loaded without it. */
export const CREW_VIEW_KINDS = ['desk', 'crew'] as const
 ...
 * slices of a whole. */
export const CREW_FILTERS = ['all', 'working', 'needs_you', 'paused'] as const
