...
/**
 * getUserMedia rejection names that mean "this specific device could not be
 ...
 * unstartable instead of degrading to the default.
 */
const FALLBACK_TO_DEFAULT = new Set([
  'OverconstrainedError', // constraints unsatisfiable — device gone
  'NotFoundError',        // no such device
  'NotReadableError',     // held by another app
  'AbortError',           // driver / hardware failure while opening
])
...
export async function acquireMicStream(exactId?: string): Promise<MediaStream> {
  ...
  try {
    ...
    if (explicit) throw e
    if (FALLBACK_TO_DEFAULT.has((e as { name?: string } | null)?.name || '')) {
...
export function humanizeMicError(e: unknown): string {
  ...
  switch (name) {
    ...
    case 'AbortError':
      // Same pair as FALLBACK_TO_DEFAULT above — if you add a name to one,
