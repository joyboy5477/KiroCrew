...
export default function SideChat({ slot }: { slot: string }) {
    ...
    onSuccess: (res, vars) => {
      ...
      if (res.steer_id) {
        ...
        if (existing && existing.content !== vars.q) {
          dispatch(sseSideQueue({ slot, action: 'edit', queue_id: existing.id, content: vars.q, raw: true }))
      ...
      if (res.queued && vars.optimistic) dispatch(sideOptimisticRollback(slot))
      if (res.queued && res.queue_id && stillQueued) {
        ...
        dispatch(sseSideQueue({ slot, action: 'edit', queue_id: res.queue_id, content: vars.q, raw: true }))
  ...
  const editQueued = useMutation({
    ...
    onSuccess: (_res, vars) => {
      ...
      dispatch(sseSideQueue({ slot, action: 'edit', queue_id: vars.queueId, content: vars.content, raw: true }))
    },
    onError: (_err, vars) => {
      ...
      {queueCards.length > 0 && (
        <div className="shrink-0 pt-1">
          <QueueStack
            ...
            onCancel={qid => { if (!blockedQueueIds.has(qid)) cancelQueued.mutate(qid) }}
            onEdit={(qid, content) => { if (!blockedQueueIds.has(qid)) editQueued.mutate({ queueId: qid, content }) }}
          />
        </div>
      )}
