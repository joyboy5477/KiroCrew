...
// the user logged into BY HAND — so anything that returns page state does not
// just show a page, it lifts authenticated content into model context. The human
...
//
// That is why VIEW is exactly ONE op. `navigate` is the only verb whose entire
// output the agent already supplied (the URL it was asked to open), so it adds
// no knowledge. Everything else returns something the agent did not have:
//   • `snapshot`/`screenshot`/`console` — the page contents, plainly.
//   • `wait_for` — its text/textGone mode evaluates a probe IN the page and
//     reports present/absent. That is an arbitrary-substring oracle, repeatable
//     to reconstruct the page a character at a time. Gating the bulk reads while
//     leaving this open would be a lock with the window next to it open.
//   • `back` — returns `{url}` from `Page.getNavigationHistory`, and the panel is
//     SHARED with the human, so that URL can be a page THEY visited. The agent
//     learns browsing history it never put there.
//
// Showing the user a page does not require any of that, so the native-default
// goal survives this split intact.
//
...
// return ANY page or history state to the agent belongs in OPERATE.
