...
function setupWindowContents(win, backendUrl) {
  ...
  win._mcAgentChannel = createAgentCommandChannel({
    ...
    dispatch: async (sessionKey, op, args) => {
      ...
      const result = await dispatchBrowserOp(entry, op, args);
      // A view-class op must not return ANYTHING derived from page state — the
      // whole reason it is ungated is that its output is only the URL the agent
      // already supplied. Enforce that at the CLASS boundary rather than trusting
      // each op's return shape, because an op can leak without changing its
      // classification: `navigate`'s `nav_timeout` error embeds
      // `waitForCommit`'s `lastState`, which carries `location.href` — so a URL
      // that never commits (HTTP 204, a `Content-Disposition: attachment`
      // download) hands back the address of whatever page was already open. The
      // panel is SHARED with the human, so that is their page, query-string
      // credentials and all — exactly the leak `back` is gated for.
      if (opClass === "view") {
        return {
          ok: !!(result && result.ok),
          ...(result && result.code ? { code: result.code } : {}),
