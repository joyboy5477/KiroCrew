 ...
 * than usual: a crew's state (claims, escalations, work log) only exists after a
 * worker has run for hours, so fixtures are the only way to photograph a
 * populated board at all.
 *
...
async function main() {
  ...
  async function load(theme, crewUi, ui, ready) {
    ...
    // Addressed by testid, never by copy: this used to wait on the words "Your
    // Desk" and broke the moment that label was renamed.
    await page.locator(ready ?? '[data-testid="crew-desk-row"]').first()
      .waitFor({ state: 'visible', timeout: 20000 })
    await page.waitForTimeout(600)
  }

  for (const theme of ['dark', 'light']) {
    await load(theme, { crewView: { kind: 'desk' }, crewFilter: 'all' })
    await page.screenshot({ path: join(OUT, `01-your-desk-${theme}.png`) })
    console.log('wrote', `01-your-desk-${theme}.png`)

    await load(theme, { crewView: { kind: 'crew', id: 'c_7f3a01' }, crewFilter: 'all' })
    await page.screenshot({ path: join(OUT, `02-crew-page-${theme}.png`) })
    console.log('wrote', `02-crew-page-${theme}.png`)

    // The create dialog, opened through the real "+" control rather than by
    // setting state, so the shot proves the control reaches it.
    await load(theme, { crewView: { kind: 'desk' }, crewFilter: 'all' })
    await page.getByRole('button', { name: /new crew/i }).first().click()
    await page.getByRole('dialog').waitFor({ state: 'visible', timeout: 10000 })
    await page.waitForTimeout(500)
    await page.screenshot({ path: join(OUT, `03-new-crew-${theme}.png`) })
    console.log('wrote', `03-new-crew-${theme}.png`)
    ...
    await page.screenshot({ path: join(OUT, `04-new-crew-lower-${theme}.png`) })
    console.log('wrote', `04-new-crew-lower-${theme}.png`)

    // 05 — the crew PROTOCOL settings, which moved off Your Desk onto the repo
    // settings page because they are repo-wide rather than per-crew.
    await load(theme, { crewView: { kind: 'desk' }, crewFilter: 'all' },
    ...
    await page.waitForTimeout(400)
    await page.screenshot({ path: join(OUT, `05-crew-protocol-settings-${theme}.png`) })
    console.log('wrote', `05-crew-protocol-settings-${theme}.png`)
  }
