...
a recognised field reports a type error, an ignored one passes silently):
    ...
    toolAliases: 12345       -> "expected a map"
"""
...
# ── The tool-alias primitive (independent of the OAuth-hint table above) ──
#
# kiro-cli exposes MCP tool names RAW, so two mounted servers exporting the same
# name leave one of them unreachable. ``mcp_tool_aliases`` computes the rename
# table; WIRING it into the agent spec needs the per-provider tool lists, which
# arrive with the Connections registry slice. The primitive lands here so the
# collision rule is pinned by tests before anything depends on it.


def test_first_server_keeps_the_bare_name_and_later_ones_are_aliased() -> None:
    aliases = mcp_tool_aliases(
        {
            "linear": ["list_teams", "list_projects", "list_issues"],
            "vercel": ["list_teams", "list_projects", "get_deployment"],
        }
    )
    assert aliases == {
    ...
    # Non-colliding tools are never renamed — an alias the user did not need is
    # a tool name the model has to be told about for no reason.
    assert not any(k.endswith(("list_issues", "get_deployment")) for k in aliases)


def test_a_single_server_needs_no_aliases() -> None:
    assert mcp_tool_aliases({"linear": ["list_teams", "list_projects"]}) == {}


def test_insertion_order_decides_which_server_keeps_the_short_name() -> None:
    """Order is the contract, so registry order stays a reviewable property."""
    reversed_order = mcp_tool_aliases(
        {"vercel": ["list_teams"], "linear": ["list_teams"]}
    )
    assert reversed_order == {"@linear/list_teams": "linear_list_teams"}


def test_empty_tool_names_are_skipped() -> None:
    assert mcp_tool_aliases({"a": ["", "x"], "b": ["", "x"]}) == {"@b/x": "b_x"}
