...
|---|---|---|
| **View** | `browser_navigate` | No |
| **Everything else** | `browser_click`, `browser_type`, `browser_press_key`, `browser_hover`, `browser_select_option`, `browser_evaluate`, `browser_snapshot`, `browser_take_screenshot`, `browser_console_messages`, `browser_wait_for`, `browser_navigate_back` | **Yes** |

`navigate` is the only ungated verb, and the reason is narrow: its whole output is
the URL you were already given, so it returns nothing you did not have. Every other
verb hands back page or history state — and the built-in browser runs on a
persistent profile holding sessions the user logged into **by hand**, so that state
is authenticated content. Two are easy to miss: `browser_wait_for`'s text mode is a
substring oracle over the live page, and `browser_navigate_back` returns a URL out of
the panel's history, which the **user** may have put there.

Opening a page for someone to look at does not require any of that.
