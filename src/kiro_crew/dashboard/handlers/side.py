...
async def api_side_queue_cancel(request: web.Request) -> web.Response:
    ...
    # Echo the caller's tab id so the other tabs can drop the card without also pasting the
    # question into their own composer. Absent or malformed is fine — consumers then fall back
    # to releasing, which is the safe direction.
    origin_client = ""
    try:
        body = await request.json()
