...
def _atomic_write(path: Path, text: str) -> None:
    """Write *text* to *path* via tmp+rename, matching session_map's durability."""
    tmp = path.with_name(path.name + ".tmp")
    tmp.write_text(text, encoding="utf-8")
    os.replace(tmp, path)


def _read_layer_b(sessions: Any, sm_key: str) -> dict[str, Any] | None:
    """Read Layer B (the kiro-cli context) for the session at *sm_key*. **Blocking.**
    ...
    Resolved through the LIVE manager (``resumable_sid``) rather than a detached
    ``SessionMap()``, so the lookup applies the same key folding resume does and
    inherits the map's self-prune (a sid whose files are gone reads as absent
    instead of producing a bundle that points at nothing).

    Returns ``{"sid", "envelope", "events"}`` or ``None`` when the session has no
    Layer B (a brand-new slot, a pruned map entry, missing files). Never raises —
    a transfer must degrade to transcript-only rather than fail.
    """
    if sessions is None:
        return None
    try:
        sid = sessions.resumable_sid(sm_key)
    except Exception:
    ...
    if not isinstance(envelope, dict) or len(events) > _MAX_LAYER_B_CHARS:
        return None
    # Egress redaction. The ``.jsonl`` is RAW kiro-cli state that can hold a
    # credential the redactors never saw (written before they existed, or carried
    # in from a channel). Unlike the display transcript, the WHOLE blob is
    # scrubbed — user turns included — because this is the model's context rather
    # than the human's shown words: silently altering a secret buried in
    # historical context is strictly safer than sending it across the boundary,
    # and the importer redacts again since it must not trust the sender.
    events, _ = redact_exfiltration_urls(events)
    events, _ = redact_credentials(events)
...
def _write_layer_b_files(layer_b: dict[str, Any], agent: str) -> str | None:
    ...
    try:
        ...
        _atomic_write(d / f"{new_sid}.json", json.dumps(envelope))
        _atomic_write(d / f"{new_sid}.jsonl", events)
        return new_sid
    except Exception:
        logger.warning(
    ...
    **fails** with :class:`SnapshotUnstable`. It deliberately does not fall back
    ...
    for _attempt in range(_SNAPSHOT_ATTEMPTS):
        ...
        bundle = await asyncio.to_thread(
            _read_and_assemble, state, key, tail, title, agent, origin, sm_key
        )
...
def _read_and_assemble(
    ...
    took on the event loop — so it is safe off-loop. *sm_key* is the session-map
    key resolved on the loop; the Layer B read (a session_map lookup plus two
    files) is blocking IO and belongs here, not on the loop.
    """
    history = _read_chained_history(state, session_key)
    history.extend(tail)
    if not history:
        history = list(tail)
    layer_b = _read_layer_b(getattr(state, "sessions", None), sm_key) if sm_key else None
    return _assemble_bundle(history, title, agent, origin, layer_b)
