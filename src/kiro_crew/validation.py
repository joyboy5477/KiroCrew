...
def _validate_crew_record_couples_phase_to_an_event(args: dict[str, Any]) -> None:
    ...
    * an escalation needs its question. ``escalation_options`` /
      ``escalation_recommendation`` alone render a Your Desk card that asks the
      human nothing.
    """
    ...
    if (
        args.get("escalation_options") or args.get("escalation_recommendation")
    ) and not args.get("escalation_question"):
        raise ValidationError("escalation_question", "required to record an escalation")
...
ISSUE_RADAR_CREW_RECORD_SCHEMA = ToolSchema(
    ...
    fields=[
        ...
        FieldSpec("escalation_question", str, max_len=MAX_MEDIUM_STRING),
        FieldSpec(
            "escalation_options",
            list,
            item_type=str,
        ...
        FieldSpec("escalation_recommendation", str, max_len=MAX_MEDIUM_STRING),
