...
def _wire_scopes(raw: object) -> list[str] | None:
    ...
    if not isinstance(raw, list) or not raw:
        return None
    if any(not isinstance(scope, str) or not scope.strip() for scope in raw):
        return None
    return list(raw)
...
def apply_kiro_oauth_hints(
    ...
    * ``_NO_SOURCE`` (argument omitted) -> nothing authoritative spoke, so
    ...
    if scopes is not _NO_SOURCE:
        wire_scopes = _wire_scopes(scopes)
...
def kiro_oauth_wire_entry(
    entry: dict[str, Any], *, store_entry: dict[str, Any] | None
) -> dict[str, Any]:
    ...
    if isinstance(store_entry, dict):
        ...
        return apply_kiro_oauth_hints(
            out,
            scopes=kiro_entry_scopes(store_entry),
            client_id=kiro_entry_client_id(store_entry),
        )

    return apply_kiro_oauth_hints(out, scopes=entry_scopes, client_id=entry_client_id)


def kiro_entry_scopes(entry: dict[str, Any]) -> list[str]:
    ...
    if INTERNAL_SCOPES_KEY in entry:
        validated = _wire_scopes(entry[INTERNAL_SCOPES_KEY])
...
def mcp_tool_aliases(tools_by_server: dict[str, list[str]]) -> dict[str, str]:
    """Aliases that de-collide identically-named tools across MCP servers.

    kiro-cli exposes MCP tool names RAW, so two servers exporting the same tool
    name leave only one reachable -- Linear and Vercel both export
    ``list_teams`` and ``list_projects``, and the model silently gets whichever
    one won.  ``toolAliases`` (an agent-spec field, ``{"@server/tool": "alias"}``)
    is kiro-cli's rename hook.

    The FIRST server holding a colliding name keeps the bare name; every later
    one is renamed to ``<server>_<tool>``.  ``tools_by_server`` insertion order
    is therefore meaningful -- pass it in registry order so which server keeps
    the short name is a stable, reviewable property of the registry rather than
    a dict-ordering accident.
    """
    seen: dict[str, str] = {}
    ...
    for server, tools in tools_by_server.items():
        for tool in tools:
            if not tool:
                continue
            owner = seen.setdefault(tool, server)
            if owner != server:
                collisions[f"@{server}/{tool}"] = f"{server}_{tool}"
    return collisions
